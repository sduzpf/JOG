...
def compress(train_loader, test_loader, model_I, model_T, train_dataset, test_dataset):
    ...
    re_BT = list([])
    re_L = list([])
    for _, (data_I, _, data_T, _, _) in enumerate(train_loader):
        var_data_I = Variable(data_I.cuda())
        with torch.no_grad():
    ...
    qu_BT = list([])
    qu_L = list([])
    for _, (data_I, _, data_T, _, _) in enumerate(test_loader):
        var_data_I = Variable(data_I.cuda())
...
class DCG(object):
    ...
    def __init__(self, k=5, gain_type='identity'):
        """
        :param k: int DCG@k
    ...
    def evaluate(self, targets, top):
        """
        :param targets: ranked list with relevance
        :return: float
        """
        gain = self._get_gain(targets)
        # discount = self._get_discount(min(top, len(gain)))
...
class NDCG(DCG):
    ...
    def _get_target(self, qu_B, re_B, qu_L, re_L, topk):
        ...
        for iter in range(num_query):
            ...
            dcg = super(NDCG, self).evaluate(gnd, topk)
            ideal = np.sort(gnd)[::-1]
            # print(ideal)
            idcg = super(NDCG, self).evaluate(ideal, topk)
            ndcg_ = dcg / idcg
            # gain = self._get_gain(gnd)
            # discount = self._get_discount(min(self.k, len(gain)))
            # topkmap_ = np.sum(np.divide(gain, discount))
